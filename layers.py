...
def conv(nin, nout, kernel_size=3, stride=1, padding=1, layer=nn.Conv2d,
         ws=False, bn=False, pn=False, activ=None, gainWS=2, groups=1):
    conv = layer(nin, nout, kernel_size, stride=stride, padding=padding, bias=False if bn else True, groups=groups)
    layers = OrderedDict()
...
class WScaleLayer(nn.Module):
    def __init__(self, incoming, gain=2):
        ...
        self.gain = gain
        self.scale = (self.gain / incoming.weight[0].numel()) ** 0.5 # numel() 返回参数个数
