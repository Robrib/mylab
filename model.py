...
class Generator(nn.Module):
    def __init__(self, max_res=8, nch=16, nc=1, bn=False, ws=False, pn=False, activ=nn.LeakyReLU(0.2), c_size=0):
        super(Generator, self).__init__()
        # resolution of output as 4 * 2^max_res: 0 -> 4x4, 1 -> 8x8, ..., 8 -> 1024x1024
        self.max_res = max_res

        # output convolutions
        self.toSPEC = nn.ModuleList()
        for i in range(self.max_res + 1):
            # max of nch * 32 feature maps as in the original article (with nch=16, 512 feature maps at max)
            self.toSPEC.append(conv(int(nch * 2 ** (8 - max(3, i))), nc, kernel_size=1, padding=0,
        ...
        self.blocks.append(nn.Sequential(OrderedDict([
            ('conv0', conv(nch * 32 + c_size, nch * 32, kernel_size=4, padding=3, bn=bn, ws=ws, pn=pn, activ=activ)),
        ...
        for i in range(self.max_res):
            ...
            self.blocks.append(nn.Sequential(OrderedDict([
                ('conv0', conv(nin, nout, bn=bn, ws=ws, pn=pn, activ=activ, groups=nout)),
                ('conv1', conv(nout, nout, bn=bn, ws=ws, pn=pn, activ=activ, groups=nout))
            ])))
...
class Discriminator(nn.Module):
    def __init__(self, max_res=8, nch=16, nc=1, bn=False, ws=False, activ=nn.LeakyReLU(0.2)):
        super(Discriminator, self).__init__()
        # resolution of output as 4 * 2^maxRes: 0 -> 4x4, 1 -> 8x8, ..., 8 -> 1024x1024
        self.max_res = max_res

        # input convolutions
        self.fromSPEC = nn.ModuleList()
        for i in range(self.max_res + 1):
            self.fromSPEC.append(conv(nc, int(nch * 2 ** (8 - max(3, i))), kernel_size=1, padding=0,
        ...
        for i in range(self.max_res):
            ...
            self.blocks.append(nn.Sequential(OrderedDict([
                ('conv0', conv(nin, nin, bn=bn, ws=ws, activ=activ, groups=nin)),
                ('conv1', conv(nin, nout, bn=bn, ws=ws, activ=activ, groups=nin))
            ])))
    ...
    def forward(self, input, x=None):
        ...
        y0 = self.fromSPEC[int(ceil(progress))](input)

        if progress % 1 != 0:
            y1 = F.avg_pool2d(input, kernel_size=2, stride=2)
            y1 = self.fromSPEC[int(progress)](y1)
...
class Q(nn.Module):
    def __init__(self, max_res=8, nch=16, nc=1, bn=False, ws=False, activ=nn.LeakyReLU(0.2), c_size=3):
        ...
        self.blocks.append(nn.Sequential(OrderedDict([
            ('conv_pool', conv(nch * 32, nch * 32, kernel_size=4, padding=0, bn=bn, ws=ws, activ=activ)),
            ('conv_class', conv(nch * 32, c_size, kernel_size=1, padding=0, ws=ws, gainWS=1, activ=None))
        ])))
        for i in range(self.max_res):
            nin = int(nch * 2 ** (8 - max(3, i + 1)))
            nout = int(nch * 2 ** (8 - max(3, i)))
...
if __name__ == '__main__':
    ...
    # print(D)
    Q = Q(max_res, nch=nch, ws=True).to(device)
    # print(Q)
    ...
    with torch.no_grad():
        ...
        print('##### Testing Q #####')
        print(f'Q has {param_number(Q)} parameters')
        for i in range((G.max_res + 1) * 2):
            print(i / 2, ' -> ', Q(G(z, i / 2), i / 2).size())
